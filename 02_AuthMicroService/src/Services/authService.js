...
class AuthService {
  ...
  async addRoleService(data) {
    try {
      ...
      const response = await jwt_helper.verifyToken(data.token);
      console.log('response of verify => ', response.email);

      const res = await authRepo.addRole({email: data.email, role: data.role});
      return res;
    } catch (error) {
  ...
  async checkRoleService(data) {
    try {
      ...
      // 1. verify token 
          const response = await jwt_helper.verifyToken(data.token);
          // console.log('response from check role by token => ', response);
      
      const res = await authRepo.checkRole(response.data);
      console.log('res => ', res);
      return res;
    } catch (error) {
  ...
  async getAllUserByRoleService(data) {
    try {
      
      // 1. verify token 
           await jwt_helper.verifyToken(data.token);

      const res = await authRepo.getAllUserByRoleRepo(data);
      return res;
    } catch (error) {
      console.log("Something went wrong in service layer (checkRoleService)");
      throw error;
  ...
  async AllUserByRoleService(data) {
    try {
        // console.log(data);
      const res = await authRepo.getAllUserByRoleRepo(data);
      return res;
