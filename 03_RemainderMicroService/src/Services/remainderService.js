...
const {createServicePayment,createServiceTicketConfirm} = require('./paymentNotiService');
  ...
  try {
    ...
    switch (service) {
      ...
      case 'CREATE_TICKET_PAYMENT':
        // console.log('get ticket payment create ',data);
        await createServicePayment(data);
        break;

       case 'CREATE_TICKET_CONFIRM':
        // console.log('get ticket payment create ',data);
        await createServiceTicketConfirm(data);
        break;
...
const deleteDataByStatusRepo = async (data) => {
  try {
    const res = await ticketRepo.deleteDataByStatus(data);
    return res;
  } catch (error) {
