pipeline{
    ...
    stages{
        stage("Clone Code"){
            steps{
            git url : "https://github.com/Saroj-kr-tharu/BookMyCinema", branch :"main"
        ...
        stage("Inject environment"){ steps{
                ...
                withCredentials( [
                    ...
                    sh '''
                        cp $GLBAL_ENV environment/.env
                        cp $APIGATEWAY_ENV environment/.env.api_gateway
                        cp $AUTH_ENV environment/.env.auth
                        cp $FORTEND_ENV environment/.env.fortend
                        cp $MONGO_ENV environment/.env.mongodb
                        cp $BOOKING_ENV environment/.env.movie_booking
                        cp $MYSQL_ENV environment/.env.mysql
                        cp $PAYMENT_ENV environment/.env.payment
                        cp $REMAINDER_ENV environment/.env.remainder
